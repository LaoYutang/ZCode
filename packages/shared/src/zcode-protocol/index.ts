...
// 动态工作流灰度门禁：workspace 级事实，host→CLI 同步；旧 CLI method-not-found → host 降级忽略。
...
// 闲时任务（Off-Peak）命令面已随账号能力移除：不再有 `offPeak/*` 方法与任务快照协议。
...
export const zcodeProtocolMethods = {
  ...
  // 动态工作流灰度门禁：workspace 级事实，host 在 agent 就绪时同步。
