...
export function createZCodeAgentService(
    ...
    const disposables = [
      ...
      client.onRequest((request) => {
        ...
        if (request.method === zcodeProtocolMethods.interactionRequestProviderRuntimeHeaders) {
          ...
          // 无账号模式：host 不再持有任何账号凭据，requestAuth 无人能产出。
          // 无应答的请求只会滞留到 CLI 侧 180s 超时，这里直接快速失败。
  ...
  async function getOrStartReadOnlyClient(
    ...
    // v4 冷恢复也才拿得到工具面。关闭时不发请求（CLI 缺省即 false，对旧 CLI/测试假客户端零打扰）。
  ...
  async function buildConversationCommandEnvelope(
    ...
    // payload.offPeakTaskId 仍作为冻结字段接受（旧客户端可能携带），但闲时轮已无专属 denylist，
    // 宿主侧不再改写 toolDisallowlist；真正的闲时轮工具策略由 CLI turn 循环负责。
    return envelope;
  }
