/* eslint-disable max-lines -- Model Provider 设置页集中编排导航、表单与模板入口，后续整体拆分时再收敛。 */
...
export function ModelProviderSection({
  ...
  const [selectedNodeKey, setSelectedNodeKey] = useState<string | null>(() =>
    initialModelProviderTarget?.providerId
  ...
  useEffect(() => {
    ...
    // selectedNodeKey、后提交 provider 列表。导航校正会把暂时不存在的 key 回退，
  ...
  const applyModelProviderTarget = useCallback(
    (target: SettingsModelProviderTarget | undefined) => {
      ...
      setSelectedNodeKey(createCustomProviderNodeKey(target.providerId));
      return true;
    },
    [],
  );
  ...
  const sectionLoading = loading || modelProvidersRefreshing;
  ...
  return (
    ...
    >
      {navigationUnavailable && !templatePickerOpen ? (
      ...
      {templatePickerOpen ? (
        ...
        <ModelProviderSectionDetail
          ...
          // Provider 的左栏排序权限被误复用成模型排序门禁，导致内置 Provider
