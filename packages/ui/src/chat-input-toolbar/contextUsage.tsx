/* eslint-disable max-lines -- context 面板聚合上下文占用、分项与缓存命中率三段展示；后续拆分需要单独梳理弹层状态边界。 */
...
const CONTEXT_PROGRESS_TONE_COLORS = [
  "var(--color-chart-1, #4f9cf9)",
  "var(--color-chart-2, #7c6cf6)",
...
/**
 * 工具栏上下文占用面板。
 * 账号套餐额度段（Coding Plan 余额、Start Plan 余额与额度重置）随登录一并移除，
 * 这里只展示本机上下文窗口占用、分项占比与缓存命中率。
 */
export function ChatContextUsage({
  ...
  return (
    ...
    >
      {/* span 承载 HoverCard 触发器的稳定锚点，避免额外浮层 ref 叠加在同一 DOM 上。 */}
      ...
      <ContextContent className="!w-80 !rounded-xl !shadow-md" side="top" sideOffset={2}>
