...
export function SessionPane({
  ...
  const handleOpenSelectionSideConversationWithPrompt = useCallback(
    async (text: string, telemetrySeed?: ConversationPromptTelemetrySeed): Promise<boolean> => {
      ...
      // 账号套餐推荐随登录移除：子会话沿用父会话继承到的模型选择，不再改写选择。
      const modelSelection = resolveSelectionSideInheritedModel(
  ...
  const dispatchSendTextAfterConfig = useCallback(
    async (
      ...
      // 账号套餐推荐随登录移除：accepted 边界不再改写模型选择，这里只是命令转发的薄封装。
  ...
  // chat dock 在挂载/卸载时共享同一个 grid 单元做上下位移淡入淡出，避免父高度突变导致硬跳。
  // prefers-reduced-motion 由 transition 组件内部降级为立即切换。
