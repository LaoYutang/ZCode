...
import {
  ...
  shouldEnableProviderAvailabilityEntryGuard,
...
import { useProviderAvailabilityEntryGuard } from "@/root/useProviderAvailabilityEntryGuard.js";
...
function RootInner({
  ...
  const providerAvailabilityEntryGuardEnabled = shouldEnableProviderAvailabilityEntryGuard();
  const { startupCheckCompleted: providerAvailabilityStartupCheckCompleted } =
    useProviderAvailabilityEntryGuard({
      enabled: providerAvailabilityEntryGuardEnabled,
      startupSyncPending: providerStartupSyncPending,
      modelSelectionView: rootModelSelectionView,
      modelSelectionError:
        rootModelSelectionRead.state.status === "error"
  ...
  const isStartupProviderEntryOpen = welcomeScreenOpenReason === "startup-provider-required";
  // 首次安装时 provider 引导入口判定晚于 workspace 注入，ChatView 会先 mount 并触发草稿预热。
  // 这里把 provider 启动检查纳入 workspace 恢复门禁，避免没有供应商配置前启动 ZCode session。
  const canRestoreWorkspaceSession =
    !isResolvingProviderStartupState && !isStartupProviderEntryOpen;
  ...
  const handleOpenAddProviderEntry = () => {
    setWelcomeScreenOpenReason("manual-add-provider");
  };
  const handleWelcomeScreenComplete = useCallback(async () => {
    ...
    try {
      ...
      logger.error("[Root] 添加供应商后创建默认 workspace 失败", {
        error,
      });
    } finally {
      setWelcomeScreenOpenReason(null);
  ...
  const settingsLayerProps = {
    ...
    allowOpenWorkspace,
    onAddProvider: handleOpenAddProviderEntry,
  };
