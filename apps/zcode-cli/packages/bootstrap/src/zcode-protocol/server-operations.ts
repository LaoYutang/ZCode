...
async function runPromptTurnInBackground(
  ...
  if (activeOffPeakTaskId) {
    // 闲时派发轮标记：只用于本轮工具 denylist（隐藏 SendMessage / Workflow）。
...
function buildPromptTurnToolDisallowlist(
  ...
  // 闲时派发轮隐藏 SendMessage / Workflow（会在本轮 modelExecution 之外重启子 Agent），
  // 与 V4 prompt-turn 及 core turn-loop-state 同值。OffPeakCreate 已随闲时任务移除，不再列入。
  if (activeOffPeakTaskId) {
...
async function createRecord(
  ...
  const app = await createWorkspaceZCodeApp(context, workspace, {
    ...
    runtimeConfig: {
      ...
      parentSessionId,
      taskType,
      // 动态工作流灰度门：本次 create/resume 参数优先，缺席时读 Host 同步到进程的
      // workspace 级结论；两者都没有就是 false（fail-closed）。这里**必须写出显式布尔**，
      // 不能省成 undefined：core 把「缺席」定义为「不参与灰度、保留全部工具」
